...
function fulltext(){
	...
	var full = function() {
		// 去掉标题中的页码
		$('h3').html($('h3').html().replace(/（\d）/, ''));
		
		$('.article-content').html(context);
		$('#show-all-cont').html('分页阅读').prevAll().hide();
		$('.article-menu').hide();
       		 imageZoom();
	}
	if(context ==''){
		...
		if (!$('.page2').length) {
			...
			var length = $('.page2').find('a').length - 3;
			var index = $('.page2').find('a').index($('.page2').find('.on'));
//			console.log(length);
